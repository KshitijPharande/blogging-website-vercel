    ...
    return (
        <div className="flex gap-2 max:lg:mb-6 max-lg:pb-6 border-grey max-lg:border-b">
        {
            Object.keys(stats).map((key, i) =>{
                return !key.includes("parent") ?<div key={i} className=
                {"flex flex-col items-center w-full h-full justify-center p-4 px-6 " + (i != 0 ? "border-grey border-1 " : "")}>
            ...
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                    <div>
                        ...
                        <p className="line-clamp-1"> Publishd on {getDay(publishedAt)}</p>
                    </div>
...
const deleteBlog = (blog, access_token, target) => {
    ...
    console.log('Deleting blog:', blog_id);  // Add log here

    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-blogs", {
        blog_id: blog_id
    }, {
        headers: {
    ...
    .then(({ data }) => {
        console.log('Blog deleted successfully:', data);  // Add log here
    ...
    .catch(err => {
        console.log('Error deleting blog:', err);  // Add log here
    });
};