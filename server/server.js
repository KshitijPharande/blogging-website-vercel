...
server.post("/google-auth", async (req, res) => {
 ...
 server.post("/change-password", verifyJWT, (req, res)=>{
    ...
    .then(user => {
        if(user.google_auth){
            return res.status(403).json({error:"You can't change password for Google account"})
        }
    ...
    .catch(err => {
        console.log(err)
    return res.status(500).json({error: "Failed to authenticate. Try with some other"
        });
    })
...
const deleteComments = (_id) =>{
    ...
    .then(comment =>{
        ...
        Notification.findOneAndUpdate({reply: _id}, { $unset: { reply: 1 } }).then(notification => console.log('reply deleted'))
...
server.post("/notifications", verifyJWT, (req, res) => {
    ...
    if (deletedDocCount) {
        ...
        skipDocs = Math.max(skipDocs, 0); // Ensure skipDocs is not negative
    }
...
server.post('/all-notifications-count', verifyJWT, (req, res) => {
    ...
    Notification.countDocuments(findQuery) // Correct method name
        .then(count => {
            return res.status(200).json({ totalDocs: count });
        })
        .catch(err => {
...
server.post("/delete-blogs", verifyJWT, (req, res) => {
    let user_id = req.user;
